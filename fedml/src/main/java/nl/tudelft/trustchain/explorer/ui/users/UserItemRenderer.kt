...
class UserItemRenderer(
    ...
    override fun bindView(item: UserItem, view: View) = with(view) {
        ...
        txtChainHeight.text = HtmlCompat.fromHtml(context.resources.getString(R.string.discovered, discoveredBlocks), 0)
        txtChainStored.text = HtmlCompat.fromHtml(context.resources.getString(R.string.stored, storedBlocks), 0)
        setOnClickListener {
            onItemClick(item)
        }
    }
