...
private val logger = KotlinLogging.logger("CustomMnistManagerTransfer")

class CustomMnistManagerTransfer(
    ...
    fun createTestBatches(): Array<Array<FloatArray>> {
        return (0 until 10).map { label ->
            val correspondingImageIndices = sampledLabels.indices
                ...
                .take(CustomMnistDataFetcherTransfer.TEST_BATCH_SIZE)
                .toTypedArray()
    ...
    companion object {
        ...
        private fun generateLabelIndexMapping(labelsArr: IntArray): Array<IntArray> {
            return (0 until CustomMnistDataFetcherTransfer.NUM_LABELS).map { label ->
